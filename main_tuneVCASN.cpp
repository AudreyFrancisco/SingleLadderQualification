...
//**NOTE**:  Need to set myChargeStart=40, myChargeStop=60 in config file!!
...
int configureChip(TAlpide *chip) {
  ...
  chip->WriteRegister (Alpide::REG_VPULSEL, 155); //was 155
}
...
//      std::cout << "Read VPULSEH : " << myVPULSEH[i] << std::endl;
  ...
  for (int istage = 0; istage < myMaskStages; istage ++) {
    ...
    for (int icharge = myChargeStart; icharge < myChargeStop; icharge ++) {
      //This is now VCASN.  NOTE/WIP:  default range is 0 to 50!!
      for (int i = 0; i < fChips.size(); i ++) {
        if (! fChips.at(i)->GetConfig()->IsEnabled()) continue;
        fChips.at(i)->WriteRegister (Alpide::VCASN, icharge); 
	//    CHANGED from REG_ITHR
      }
      usleep (5000);
...
int main() {
  ...
  if (fBoards.size() == 1) {
    ...
    std::cout << "Found " << fEnabled << " enabled chips out of " << fChips.size() << "." << std::endl;
    ...
    sprintf(fName, "Data/TuneVScan_%s.dat", Suffix);
    WriteDataToFile (fName, true);
    sprintf(fName, "Data/VScanConfig_%s.cfg", Suffix);
