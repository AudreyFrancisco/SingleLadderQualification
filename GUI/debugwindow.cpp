    ...
    ui(new Ui::DebugWindow)
{
    ...
    connect(ui->configLoadButton, SIGNAL(clicked()),
        this, SLOT(selectAndLoadConfig()));
    connect(this, SIGNAL(selectScan(TScanType)),
    	this->parent(), SLOT(doDebugScan(TScanType)));
    connect(this, SIGNAL(passConfigFile(QByteArray)),
    	this->parent(), SLOT(loadConfigFile(QByteArray)));
    connect(this->parent(), SIGNAL(deviceLoaded(TDeviceType)),
    	this, SLOT(displayDeviceAndScans(TDeviceType)));
}
