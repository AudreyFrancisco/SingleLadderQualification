...
void scanCurrentDac(TAlpide *chip, Alpide::TRegister ADac, Alpide::TRegister adc, const char *Name,
  ...
  if (!myDAQBoard) { // MOSAIC board internal ADC read
    for (unsigned int value = 0; value < 256; value += sampleDist) {
      for (unsigned int repetition = 0; repetition < sampleRepetition; ++repetition) {
        ...
        Current = chip->ReadDACCurrent(adc);
        printf("%d %.3f\n", value, Current);
        fprintf(fp, "%d %.3f\n", value, Current);
      }
    }
...
void scanVoltageDac(TAlpide *chip, Alpide::TRegister ADac, Alpide::TRegister adc, const char *Name,
                    unsigned int sampleDist = 1, unsigned int sampleRepetition = 1,
                    string suffix = "")
{
  char     fName[50];
  float    Voltage, VoltageSelf;
  uint16_t old;
  ...
  if (!myDAQBoard) { // MOSAIC board internal ADC read
    for (unsigned int value = 0; value < 256; value += sampleDist) {
      for (unsigned int repetition = 0; repetition < sampleRepetition; ++repetition) {
        chip->WriteRegister(ADac, value);
        usleep(1000);
        VoltageSelf = chip->ReadDACVoltage(ADac);
        Voltage     = chip->ReadDACVoltage(adc);
        fprintf(fp, "%d %.3f %.3f\n", value, VoltageSelf, Voltage);
        printf("%d %.3f %.3f\n", value, VoltageSelf, Voltage);
      }
    }
  }
                ...
                string suffix = "")
{
  ...
  std::cout << "ChipID = " << chip->GetConfig()->GetChipId() << "    Scan ADC DAC" << std::endl;
...
int main(int argc, char **argv)
{
  ...
  if (fBoards.size()) {
    ...
    for (unsigned int i = 0; i < fChips.size(); i++) {
      ...
      fChips.at(i)->WriteRegister(Alpide::REG_VCASN, 55);
      fChips.at(i)->WriteRegister(Alpide::REG_VCLIP, 0);
      fChips.at(i)->WriteRegister(Alpide::REG_BYPASS_BUFFER, 0x08);
      usleep(2000);
      ...
      // mySampleRepetition,
      //               Suffix);
      // scanVoltageDac(fChips.at(i), Alpide::REG_VCASP, "VCASP", mySampleDist, mySampleRepetition,
      //               Suffix);
      // scanVoltageDac(fChips.at(i), Alpide::REG_VCASN, "VCASN", mySampleDist, mySampleRepetition,
      //               Suffix);
      ...
      // mySampleRepetition,
      //               Suffix);
      scanVoltageDac(fChips.at(i), Alpide::REG_VRESETP, Alpide::REG_VCLIP, "VRESETP_VCLIP",
                     mySampleDist, mySampleRepetition, Suffix);
      fChips.at(i)->WriteRegister(Alpide::REG_VRESETP, 117);
      scanVoltageDac(fChips.at(i), Alpide::REG_VCLIP, Alpide::REG_VRESETP, "VCLIP_VRESETP",
                     mySampleDist, mySampleRepetition, Suffix);
      // scanVoltageDac(fChips.at(i), Alpide::REG_VTEMP, "VTEMP", mySampleDist, mySampleRepetition,
      //               Suffix);

      // scanCurrentDac(fChips.at(i), Alpide::REG_IAUX2, "IAUX2", mySampleDist, mySampleRepetition,
      //               Suffix);
      // scanCurrentDac(fChips.at(i), Alpide::REG_IRESET, "IRESET", mySampleDist,
      // mySampleRepetition,
      //               Suffix);
      // scanCurrentDac(fChips.at(i), Alpide::REG_IDB, "IDB", mySampleDist, mySampleRepetition,
      //               Suffix);
      // scanCurrentDac(fChips.at(i), Alpide::REG_IBIAS, "IBIAS", mySampleDist, mySampleRepetition,
      //               Suffix);
      // scanCurrentDac(fChips.at(i), Alpide::REG_ITHR, "ITHR", mySampleDist, mySampleRepetition,
      //               Suffix);

      // scanADCDac(fChips.at(i), mySampleDist, mySampleRepetition, Suffix);

      // char fName[200];
      // snprintf(fName, 200, "Data/AVDD_Chip%d_%d_%s.dat", fChips.at(i)->GetConfig()->GetChipId(),
      //         fChips.at(i)->GetConfig()->GetCtrInt(), Suffix);
      // FILE *fp = fopen(fName, "w");
      ...
      //  theValue = 2. * ((float)theResult - (float)(fChips.at(i)->GetADCOffset())) *
      ...
      //// Temperature
      // snprintf(fName, 200, "Data/TEMP_Chip%d_%d_%s.dat", fChips.at(i)->GetConfig()->GetChipId(),
      //         fChips.at(i)->GetConfig()->GetCtrInt(), Suffix);
      // fp = fopen(fName, "w");
