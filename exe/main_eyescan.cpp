...
namespace eyescan {
  ...
  class EyeScan {
  public:
    EyeScan(TReadoutBoardMOSAIC &board, size_t chipId, std::string filename, int hStep = 1,
            int vStep = 1, bool verbose = false)
    ...
    void runHScan(double *ber, int vOffset)
    {
      ...
      for (int hOffset = hMin; hOffset < 0; hOffset += hStep) {
        ...
        b = ber[idx] = BERmeasure(hOffset, vOffset, MAX_PRESCALE);
    ...
    void runFullScan()
    {
      ...
      const double        minMeasure = ((double)1.0 / ((double)BUS_WIDTH * (double)0xffff *
