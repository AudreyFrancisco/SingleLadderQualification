...
void componentActivities(AlpideDB *theDB)
{
  int err;
  int ComponentID;
  cout << endl << "------ Read Component Activities ----------" << endl;
  printf(" Input the Component id :");
  err = scanf("%d", &ComponentID);
  if (err != 1) {
    printf(" Failed to read the Activity  id ");
  ...
  ComponentDB *                     theCompTable = new ComponentDB(theDB);
  vector<ComponentDB::compActivity> Histo;
  AlpideTable::response *theResult = theCompTable->GetComponentActivities(ComponentID, &Histo);
  cout << theCompTable->DumpResponse(theResult) << endl;
  for (unsigned int i = 0; i < Histo.size(); i++) {
    cout << endl << Histo.at(i).ID << "\t" << Histo.at(i).Name;
  }
}
