...
void InitScanParameters()
{
  ...
  // myMaskStages     = 20; //fConfig->GetScanConfig()->GetParamValue("NMASKSTAGES");
  // myPixPerRegion   = 16; // fConfig->GetScanConfig()->GetParamValue("PIXPERREGION");
  myNTriggers      = 50; // fConfig->GetScanConfig()->GetParamValue("NINJ");
  maxTrigsPerTrain = 2;  // fConfig->GetScanConfig()->GetParamValue("MAXNTRIGTRAIN");
}
...
void scan()
{
  ...
  std::vector<uint8_t> frameBuffer(1 * 1024 * 1024);
  int n_bytes_data, n_bytes_frame, n_bytes_header, n_bytes_trailer, errors8b10b = 0,
                                                                    nClosedEvents = 0;
  int                   nBad                                                      = 0;
  int                   nSkipped                                                  = 0;
  int                   prioErrors                                                = 0;
  ...
  std::vector<std::vector<int>> binMatrix =
      AlpideConfig::GetScaledBinaryMatrix("../exe/qr_binary.txt");
  ...
  for (int istage = 0; istage < 512; istage++) {
    for (int col = 0; col < 1024; col++) {
      std::cout << std::endl << "Mask stage " << istage << std::endl;
      for (unsigned int i = 0; i < fChips.size(); i++) {
        if (!fChips.at(i)->GetConfig()->IsEnabled()) continue;
        AlpideConfig::WritePixRegSingle(fChips.at(i), Alpide::PIXREG_MASK, false, istage, col);
        AlpideConfig::WritePixRegSingle(fChips.at(i), Alpide::PIXREG_SELECT, true, istage, col);
      }
      ...
      for (int itrain = 0; itrain < nTrains; itrain++) {
        ...
        for (unsigned int ib = 0; ib < fBoards.size(); ++ib) {
          ...
          boardaroo->LatchAllCounters();
          uint16_t lane_timeout = boardaroo->datapathmon->Read(17 * 8 + 12 + 2);
          if (lane_timeout != 0)
            std::cout << "WARNING: LANE PACKAGER LANE TIMEOUT: " << (lane_timeout & 0xff)
                      << " AND GBT PACKER LANE TIMEOUT: " << ((lane_timeout >> 8) & 0xff)
                      << std::endl;
          ...
          while (itrg < nTrigsPerTrain * fEnabled) {
            ...
            while (!(fBoards.at(ib)->ReadEventData(n_bytes_data, buffer.data()) == 0) &&
                   trial != maxTrials) { // no event available in buffer yet, wait a bit
              usleep(10000);
              trial++;
            }
            boardaroo->ReadFrameData(n_bytes_frame, frameBuffer.data());
            if (trial == maxTrials) {
              std::cout << "MAX TRIALS REACHED, GIVING UP" << std::endl;
              nSkipped++;
              ...
              BoardDecoder::DecodeGbtFrame(frameBuffer.data(), n_bytes_frame, frameInfo);
              ...
              if (!AlpideDecoder::DecodeEvent(buffer.data() + n_bytes_header, n_bytes_chipevent,
                                              Hits, 0, boardInfo.channel, prioErrors, hitLimit,
                                              Stuck, chipId, bc)) {
          ...
          //  std::cout << i << ":\t region: " << Hits->at(i).region << "\tdcol: " <<
          //  Hits->at(i).dcol
  ...
  if (fBoards.size()) {
    ...
    for (const auto &rBoard : fBoards) {
      ...
      //}
        ...
        rBoard->SetTriggerConfig(true, true, 1000, 1000);
