...
TRuDctrlModule::TRuDctrlModule(TReadoutBoardRU &board, uint8_t moduleId, bool logging)
    : TRuWishboneModule(board, moduleId, logging), m_connector(0), m_connectorSet(false) {}

void TRuDctrlModule::WriteChipRegister(uint16_t Address, uint16_t Value, uint8_t chipId,
                                       bool commit) {
...
int TRuDctrlModule::ReadChipRegister(uint16_t Address, uint16_t &Value, uint8_t chipId) {
  ...
  if (results.size() != 2) {
    if (m_logging)
      std::cout << "TReadoutBoardRU: ChipRead: expected 2 results, got " << results.size() << "\n";
    return -1;
  }
  ...
  if (status != TRuDctrlModule::CHIP_READ_STATUS_OK) {
    if (m_logging)
      std::cout << "TReadoutBoardRU: ChipRead: Return status NOK, got 0x" << std::hex << (int)status
                << ", expected: " << (int)TRuDctrlModule::CHIP_READ_STATUS_OK << std::dec << "\n";
    return -1;
  }
  if (chipid_read != chipId) {
    if (m_logging)
      std::cout << "TReadoutBoardRU: ChipRead: ChipID read NOK, expected " << (int)chipId
                << ", got " << (int)chipid_read << "\n";
    return -2;
  }
