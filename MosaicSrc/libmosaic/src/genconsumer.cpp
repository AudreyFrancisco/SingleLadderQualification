...
// printf("Called GenConsumer::parse ne:%d from buffer at 0x%08x\n", numClosed, (unsigned long) p);
	...
	for (int i=0; i<numClosed; i++){
		for (int j=0; j<eventSize; j+=4){
			d = MBoard::buf2ui(p);
			if (d!=expectedData){
//				printf("ERROR: GenConsumer::parse found:0x%08x exp:0x%08x at 0x%08lx\n", d, expectedData, (unsigned long) p);
//				printf("ERROR: GenConsumer::parse numClosed:%d eventSize:%d j:%d\n", numClosed, eventSize, j);
