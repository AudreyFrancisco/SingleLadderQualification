...
def generateTheEOStransferScript(ServiceAccount, LocalBasePath):
    ...
    file.write("# --------------- LOCK FILE MECHANISM ------------\n")
    file.write("LOCKFILE=/tmp/EOStransferlock.loc\n")
    file.write("if [ -e ${LOCKFILE} ] && kill -0 `cat ${LOCKFILE}`; then\n")
    file.write("    # echo \"already running\"\n")
    file.write("    exit 0\n")
    file.write("fi\n")
    file.write("# make sure the lockfile is removed when we exit and then claim it\n")
    file.write("trap \"rm -f ${LOCKFILE}; exit\" INT TERM EXIT\n")
    file.write("echo $$ > ${LOCKFILE}\n")
    file.write("# ------------------------------------------------\n")
    ...
    file.write("echo \" ------ ALICE-ITS EOS Repo sync program - v.2.1 - A.Franco - INFN BARI Italy\" \n")
    ...
    file.write("     rsync --files-from=$INCLUDEFILE -Lrauze ssh $DBATTACHBASEPATH $DBATTACHREMOTEPATH\n")
...
def generateTheEOSAllSyncScript(ServiceAccount, LocalBasePath):
    ...
    file.write("  ERRORS=0\n") 
    file.write("  # - do the sync\n") 
    file.write("  #loops in tree for tests folder\n") 
    file.write("  for HIC in $DBATTACHBASEPATH/*; do  if [[ -d $HIC ]]; then\n") 
    file.write("    for TES in ${HIC}/*; do if [[ -d $TES ]]; then\n") 
    ...
    file.write("          # - evaluates the result\n") 
    file.write("          if [[ $? -gt 0 ]]; then\n") 
    file.write("            STOPDATE=`date` \n") 
    file.write("            echo \"$STOPDATE : Error to sync $SOURCE on the remote repository $DESTINATION\" >> $LOGFILENAME\n") 
    file.write("            echo \"$STOPDATE : Error to sync $SOURCE on the remote repository $DESTINATION\"\n") 
    file.write("            let \"ERRORS=$ERRORS+1\"\n") 
    file.write("          else\n") 
    file.write("            STOPDATE=`date`\n") 
    file.write("            echo \"Remote repository $DESTINATION sync done at $STOPDATE !\"\n") 
    file.write("          fi\n") 
    file.write("          # ---- skip to the nest\n") 
    file.write("          sleep 15\n") 
    file.write("        fi done\n") 
    file.write("      fi done\n") 
    file.write("    fi done\n") 
    file.write("  fi done\n") 
    file.write("  # evaluates if there are errors\n") 
    file.write("  STOPDATE=`date`\n") 
    file.write("  if [ $ERRORS -eq 0 ]; then\n") 
    file.write("    # We finished !\n") 
    file.write("    echo \"Remote repository sync done. ($STARTDATE -> $STOPDATE)\" >> $LOGFILENAME\n") 
    file.write("    echo \"Remote repository sync done at $STOPDATE !\"\n") 
    file.write("    echo \" --------- \"\n") 
    file.write("    exit 0\n") 
    file.write("  else\n") 
    file.write("    let \"SYNCATTEMPTS=$SYNCATTEMPTS-1\" \n") 
    file.write("    echo \"$STOPDATE : Errors to sync, retray... (remain att=$SYNCATTEMPTS, Errors=$ERRORS, At=$DBATTACHREMOTEPATH )\" \n") 
    file.write("    echo \"$STOPDATE : Errors to sync, retray... (remain att=$SYNCATTEMPTS, Errors=$ERRORS, At=$DBATTACHREMOTEPATH )\" >> $LOGFILENAME\n") 
    file.write("  fi\n") 
    file.write("done\n") 
    file.write("STOPDATE=`date`\n") 
    file.write("echo \"$STOPDATE : Exit for error. Abort Sync !\"\n") 
    file.write("echo \"$STOPDATE : Exit for error. Abort Sync !\" >> $LOGFILENAME\n") 
    file.write("echo \" --------- \"\n") 
    file.write("exit 1\n") 
...
def generateTheCronJobControlScript(ServiceAccount, LocalBasePath):
    ...
    file.write("\n")
    file.write("sudo rm -f $HINIBITFILE\n")
    file.write("LOCKFILE=/tmp/EOStransferlock.loc\n")
    file.write("sudo rm -f $LOCKFILE\n")
    file.write("exit 0\n")
    file.write(" \n")
    file.close()
    os.chmod(theScriptFileName, 0777)
    ...
    file.write("echo \" ------ ALICE-ITS EOS Repo sync CronJob control - v.1.0 - A.Franco - INFN BARI Italy\" \n")
    file.write("\n")
    file.write("sudo touch $HINIBITFILE\n")
    file.write("LOCKFILE=/tmp/EOStransferlock.loc\n")
    file.write("sudo rm -f $LOCKFILE\n")
    file.write("exit 0\n")
    file.write(" \n")
