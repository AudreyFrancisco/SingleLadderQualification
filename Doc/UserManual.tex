...
\item \textbf{Ubuntu 16.04 LTS:}
  \begin{lstlisting}
    apt-get update -y && \
    apt-get install -qq -y man git subversion dpkg-dev make cmake clang-format \
...
\item \textbf{CentOS CERN 7:}
  \begin{lstlisting}
    yum install -y gcc gcc-c++ make cmake tar zlib wget subversion clang \
  ...
  \begin{lstlisting}
    source /opt/root/bin/thisroot.sh .
  \end{lstlisting}
  Activate the \texttt{llvm-toolset-7}
  \begin{lstlisting}
...
\item \textbf{Scientific Linux CERN 6 *DEPRECATED*:}
  \begin{lstlisting}
    ...
    yum install -y git cmake binutils which clang \
