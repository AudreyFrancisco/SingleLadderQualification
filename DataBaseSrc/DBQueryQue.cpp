...
{
  return ((isLocal ? theLocalCopyRepoPath : theBaseRepoPath) + "/" + theSpecificQueuePath + "/" +
          FileName);
}
...
bool DBQueryQueue::__makeTheFileName(const string Name, string *QueryFileName, bool IsLocal)
{
  ...
  CompletePath =
      (IsLocal ? theLocalCopyRepoPath : theBaseRepoPath) + "/" + theSpecificQueuePath + "/";
  if (!pathExists(CompletePath)) {
    cerr << "DBQueue Error : the Path to locate the Queue not exists ! (" << CompletePath << ")"
         << endl;
    return (false);
  }
...
bool DBQueryQueue::Serialize(string QueryType, ActivityDB::activity *activity, string *Output)
{
  ...
  for (int i = 0; i < activity->Attachments.size(); i++) {
    ...
    cJSON_AddStringToObject(rec, "LocalFileName",
                            activity->Attachments.at(i).LocalFileName.c_str());
    cJSON_AddStringToObject(rec, "RemoteFileName",
                            activity->Attachments.at(i).RemoteFileName.c_str());
  }
  arr = cJSON_CreateArray();
  ...
  while (theState != ST_DONE) {
    ...
    case ST_CREATECOMPIN: // Assign theComponents
      if (act->theAsyncronuosQueue->Read(theFileName, &theQueryType, activ)) {
        ...
        for (i = theSubState; i < activ->OutComps.size(); i++) {
          respo =
              act->AssignComponent(activ->ID, activ->OutComps.at(i).CompID,
    ...
    case ST_CLOSED: // Remove the element from the queue
      if (act->theAsyncronuosQueue->Pop(&theQueryType, activ)) {
        ...
        cout << "DBQueueConsumer : Asyncronous Activity created :" << activ->ID << "  "
             << activ->Name << endl;
      }
    }
    __checkTimeOut();
 ...
 *
 *---------------- */
vector<string> DBQueueConsumer::GetQueue() { return (act->theAsyncronuosQueue->GetTheQueue()); }
