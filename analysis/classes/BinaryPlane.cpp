...
ClassImp(BinaryPlane)
    ...
    : TObject(), fNClusters(0), fNPixelsX(-1), fNPixelsY(-1), fPlaneID(-1), fPosX(-1.), fPosY(-1.),
      fPosZ(-1.), fNHitPix(-1), fClusters(0)
{
...
BinaryPlane::BinaryPlane(const BinaryPlane &orig)
    : TObject(orig), fNClusters(0), fNPixelsX(-1), fNPixelsY(-1), fPlaneID(-1), fPosX(-1.),
      fPosY(-1.), fPosZ(-1.), fNHitPix(-1), fClusters(0)
{
  fClusters    = new TClonesArray(BinaryCluster::Class());
  fChipName[0] = 0;
