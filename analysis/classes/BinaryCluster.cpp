...
ClassImp(BinaryCluster)
    ...
    : TObject(), fClusterID(-1), fSectorID(-1), fNPixels(0), fPixels(0x0)
{
}
...
BinaryCluster::BinaryCluster(Int_t npixels)
    : TObject(), fClusterID(-1), fSectorID(-1), fNPixels(npixels)
{
...
BinaryCluster::BinaryCluster(const BinaryCluster &orig)
    : TObject(), fClusterID(orig.fClusterID), fSectorID(orig.fSectorID), fNPixels(orig.fNPixels)
{
  fPixels = new BinaryPixel[fNPixels];
...
Float_t BinaryCluster::GetMaxSpread()
{
  ...
  for (Int_t i = 0; i < fNPixels; ++i) {
    for (Int_t j = i + 1; j < fNPixels; ++j) {
      Float_t d = TMath::Sqrt((fPixels[i].GetCol() - fPixels[j].GetCol()) *
                                  (fPixels[i].GetCol() - fPixels[j].GetCol()) +
                              (fPixels[i].GetRow() - fPixels[j].GetRow()) *
                                  (fPixels[i].GetRow() - fPixels[j].GetRow()));
      if (d > max) max = d;
    }
  }
...
{
  for (Int_t i = 0; i < fNPixels; ++i)
    if (fPixels[i].IsHot()) return kTRUE;
  return kFALSE;
}
...
{
  for (Int_t i = 0; i < fNPixels; ++i)
    if (fPixels[i].IsBorder()) return kTRUE;
  return kFALSE;
}
...
{
  for (Int_t i = 0; i < fNPixels; ++i)
    if (fPixels[i].GetFlag(2)) return kTRUE;
  return kFALSE;
}
