...
void ReadFile(const char *filepath, const char *fNameChip, TH2F *hHitmap,
              ...
              std::vector<std::vector<std::vector<double>>> &moduleHitsVector)
{
  ...
  while (fscanf(fp, "%d %d %d %d %d", &mod, &chip, &col, &row, &hits) == 5) {
    ...
    moduleHitsVector[(int)mod - 1].push_back(std::vector<double>(4));
    moduleHitsVector[(int)mod - 1].back()[0] = hits;
    moduleHitsVector[(int)mod - 1].back()[1] = chip;
    moduleHitsVector[(int)mod - 1].back()[2] = col;
    moduleHitsVector[(int)mod - 1].back()[3] = row;
  }
  rewind(fp);
...
int OB_mask_maker(TString directory, int nInj = -1)
{
  ...
  sprintf(maskdirname, "%s/mask", filepath);
  mkdir(maskdirname, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
  ...
  for (int imod = 1; imod < 8; imod++) {
    ...
    for (int ichip = 0; ichip < 7; ichip++) {
      ...
      ReadFile(filepath, fNameChip, hHitmap, hlist, ichip, nInj, m_hist, modHitsVect);
      sprintf(fNameChip, "%s/Source_Chip%d_%d.dat", filepath, imod, ichip + 8); // second row
      ReadFile(filepath, fNameChip, hHitmap, hlist, ichip + 8, nInj, m_hist, modHitsVect);
    }


    hHitmap->SetOption("colz");

    std::ofstream output_file(Form("%s/mask/mask_mod_%d.dat", filepath, imod));
    std::sort(modHitsVect[imod - 1].begin(), modHitsVect[imod - 1].end(),
              [](std::vector<double> a, std::vector<double> b) { return a[0] > b[0]; });
    for (auto &d1 : modHitsVect) {
      if (d1.size() > level) {
        d1.resize(level);
      }
      else
