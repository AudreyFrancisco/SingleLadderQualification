...
void analyse_AVDD(TString inputFile)
{
  ...
  if (!t) {
    std::cerr << "Could not find the TTree \"AVDDsummary\" in the input file " << inputFile.Data()
              << "!" << std::endl;
  }
  ...
  if (!f_out_fit.is_open()) {
    std::cerr << "Could not open the output file for the fit results: " << f_out_fit_name.Data()
              << "!" << std::endl;
  }

  TFile *f_out = new TFile(TString::Format("%s_output.root", inputFileBaseName.Data()), "RECREATE");

  TString draw_str[] = {"AVDDsetV:AVDDmeasV",
                        "AVDDsetV:VTEMPmeasV",
                        "AVDDmeasV:VTEMPmeasV",
                        "AVDDsetV:VTEMPmeasV/0.772+0.023",
                        "AVDDmeasV+0.023:VTEMPmeasV/0.772+0.023",
                        "AVDDsetV:VTEMPmeasV/0.772+0.023-AVDDsetV",
                        "AVDDmeasV+0.023:VTEMPmeasV/0.772+0.023-AVDDmeasV+0.023"};
  TString name_str[] = {"gAVDDmeasVvsAVDDsetV_Chip%d",     "gVTEMPmeasVvsAVDDsetV_Chip%d",
                        "gVTEMPmeasVvsAVDDmeasV_Chip%d",   "EstimatedAVDDvsAVDDsetV_Chip%d",
                        "EstimatedAVDDvsAVDDmeasV_Chip%d", "EstDevAVDDvsAVDDsetV_Chip%d",
                        "EstDevAVDDvsAVDDmeasV_Chip%d"};
  TString axes_str[] = {
      ";AVDD_{set} (V); AVDD_{meas} (V)",       ";AVDD_{set} (V);VTEMP_{meas} (V)",
      ";AVDD_{meas} (V);VTEMP_{meas} (V)",      ";AVDD_{meas} (V);VTEMP_{meas} (V)",
      ";AVDD_{meas} (V);VTEMP_{meas} (V)",      ";AVDD_{set} (V);# Delta AVDD_{set} (V)",
      ";AVDD_{meas} (V);#Delta AVDD_{meas} (V)"};
  Float_t fit_low_lim[] = {1.500, 1.60, 1.60, 1.60, 1.60, 1.60, 1.60};
  Float_t fit_up_lim[]  = {1.721, 2.05, 1.701, 2.01, 1.701, 2.01, 1.701};
  TString func_str[]    = {
      "[1]*x+[0]",           "[0]*x",           "[0]*x",           "[1]*(x-1.8)+[0]+1.8",
      "[1]*(x-1.8)+[0]+1.8", "[1]*(x-1.8)+[0]", "[1]*(x-1.8)+[0]",
  };
  ...
  for (unsigned int i_chip = 0; i_chip < 9; ++i_chip) {
    ...
    for (unsigned int i_draw_str = 0; i_draw_str < sizeof(draw_str) / sizeof(TString);
         ++i_draw_str) {
      ...
      TF1 *func = new TF1("func_lin", func_str[i_draw_str].Data(), fit_low_lim[i_draw_str],
                          fit_up_lim[i_draw_str]);
