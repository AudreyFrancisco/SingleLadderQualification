...
void analyse_DAC(TString inputFile, TString DAC, Int_t chip = -1)
{
  ...
  if (!t) {
    std::cerr << "Could not find the TTree \"DACsummary\" in the input file " << inputFile.Data()
              << "!" << std::endl;
  }
  ...
  std::cout << "#Voltage\tExpecteValue\tMeasMean\tMeasRMS\tDevMean\tDevMean (%)\tMeasured "
               "Voltage\tMeasured Deviation"
            << std::endl;
  ...
  for (float voltage = 1.30; voltage < 2.00; voltage += 0.02) {
    ...
    for (unsigned int i_chip = 0; i_chip < 9; ++i_chip) {
      ...
      cut_str_mod +=
          TString::Format("&&(Voltage>%0.3f)&&(Voltage<%0.3f)", voltage - 0.001, voltage + 0.001);
      ULong64_t n = t->Draw(draw_str.Data(), cut_str_mod.Data(), "goff");
      if (n == 0) continue;
      float meas_value = t->GetV2()[0];
      float dev        = expected_value - meas_value;
      // std::cout << voltage << "\t" << i_chip << "\t" << expected_value << "\t" << meas_value <<
      // "\t" << dev << "\t" << dev/expected_value*100. << std::endl;
      mean += meas_value / 9.;
