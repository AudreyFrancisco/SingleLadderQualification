...
Bool_t Monitoring::InitHistos()
{
  ...
  maptot = new TH2F("allevt", "pALPIDEfs Multiple Events;Column;Row", 1024, -0.5, 1023.5, 512, -0.5,
                    512);
...
Bool_t Monitoring::ProcessSingleEvent()
{
  ...
  if (!palpidefs->ReadEvent()) {
    cout << "Monitoring::problem Reading event:" << palpidefs->GetEventCounter() << endl;
    return kFALSE;
  }
...
//_______________________________________________________________
void Monitoring::Stop() { fStop = kTRUE; }
//_______________________________________________________________
