...
Bool_t analysis_basic(const TString filepath_tree, // input tree path
                      ...
                      )
{
  ...
  if (!chain->Add(filepath_tree.Data())) {
    cerr << "analysis_basic() : ERROR: Cannot find event tree in file! " << filepath_tree.Data()
         << endl;
    return kFALSE;
  }
  ...
  TFile *file_plots =
      new TFile(Form("%s/%s.root", dirpath_plots.Data(), filename_out.Data()), "RECREATE");
  if (!file_plots->IsOpen()) {
    cerr << "analysis_basic() : ERROR: Cannot open output plots file! " << dirpath_plots.Data()
         << endl;
    return kFALSE;
  }

  TH2F *hPixHits = new TH2F("hPixHits", "Pixel hit map;Column;Row;Frequency", n_secs * scols, -0.5,
                            n_secs * scols - 0.5, srows, -0.5, srows - 0.5);
  hPixHits->SetStats(0);
  TH2F *hCluHits = new TH2F("hCluHits", "Cluster hit map;Column;Row;Frequency", n_secs * scols,
                            -0.5, n_secs * scols - 0.5, srows, -0.5, srows - 0.5);
  hCluHits->SetStats(0);
  TH1F *hCluX = new TH1F("hCluX", "Clusters X-position;Column;Frequency", n_secs * scols, -0.5,
                         n_secs * scols - 0.5);
  hCluX->SetStats(0);
  TH1F *hCluY =
      new TH1F("hCluY", "Clusters Y-Position;Row;Frequency", srows - 0.5, -0.5, srows - 0.5);
  hCluY->SetStats(0);
  ...
  for (Short_t i = 0; i < n_secs; ++i) {
    hNPix[i] = new TH1F(
        Form("hNPix_%i", i),
        Form("Number of hit pixels per event, sector %i;Number of hit pixels per event;Frequency",
             i),
        3000, -0.5, 3000 - 0.5);
    hNClu[i] = new TH1F(
        Form("hNClu_%i", i),
        Form("Number of clusters per event, sector %i;Number of clusters per event;Frequency", i),
        1500, -0.5, 1500 - 0.5);
    hMult[i] = new TH1F(Form("hMult_%i", i),
                        Form("Cluster size, Sector %i;Number of pixels in cluster;Frequency", i),
                        max_mult + 1, -1.5, max_mult - 0.5);
    // hMult[i]->SetStats(0);
    hXSpread[i] =
        new TH1F(Form("hXSpread_%i", i),
                 Form("Cluster X Spread, Sector %i;Cluster X Spread (pixels);Frequency", i),
                 max_spread + 1, -1.5, max_spread - 0.5);
    hXSpread[i]->SetStats(0);
    hYSpread[i] =
        new TH1F(Form("hYSpread_%i", i),
                 Form("Cluster Y Spread, Sector %i;Cluster Y Spread (pixels);Frequency", i),
                 max_spread + 1, -1.5, max_spread - 0.5);
    hYSpread[i]->SetStats(0);
    hMaxSpread[i] =
        new TH1F(Form("hMaxSpread_%i", i),
                 Form("Cluster Max Spread, Sector %i;Cluster Max Spread (pixels);Frequency", i),
                 max_spread + 1, -1.5, max_spread - 0.5);
  ...
  for (Long_t ientry = 0; ientry < nentries; ++ientry) {
    ...
    for (Short_t isec = 0; isec < n_secs; ++isec) {
      ...
      for (Int_t iclu = 0; iclu < event->GetPlane(isec)->GetNClustersSaved(); ++iclu) {
        ...
        hCluHits->Fill(cluster->GetX(), cluster->GetY());
        ...
        //}
        // if(iclu > 0)
        //  hPixHits->Fill(event->GetPlane(isec)->GetCluster(iclu)->GetY(),
        //  event->GetPlane(isec)->GetCluster(iclu-1)->GetY());
      }
    } // END FOR sectors
  }   // END FOR entries
