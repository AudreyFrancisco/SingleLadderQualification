/* Written by Miljenko Suljic, m.suljic@cern.ch */
#include "Riostream.h"
#include "TFile.h"
#include "TChain.h"
#include "TH2F.h"
#include "TMath.h"
#include "TCanvas.h"
#include "TString.h"

#include "../classes/BinaryEvent.hpp"

TH2F* draw_cluster(BinaryCluster* cluster, Short_t cols, Short_t rows) {
    TH2F* h = NULL;
    BinaryPixel* pixels = cluster->GetPixelArray();
    Short_t max_x=0, max_y=0, min_x=cols, min_y=rows, dx, dy;
    for(Int_t ipix=0; ipix < cluster->GetMultiplicity(); ++ipix) {
        Short_t pix_x = pixels[ipix].GetCol();
        Short_t pix_y = pixels[ipix].GetRow();
        if( pix_x > max_x) max_x = pix_x;
        if( pix_y > max_y) max_y = pix_y;
        if( pix_x < min_x) min_x = pix_x;
        if( pix_y < min_y) min_y = pix_y;
    }
    if( (max_x-min_x)%2 ) dx = max_x-min_x+1;
    else                  dx = max_x-min_x;
    if( (max_y-min_y)%2 ) dy = max_y-min_y+1;
    else                  dy = max_y-min_y;
    Float_t x = TMath::Ceil(cluster->GetX());
    Float_t y = TMath::Ceil(cluster->GetY());
    h = new TH2F("hclust", "Cluster form;Colum;Row",
                 dx+6, x-dx/2.-3.5, x+dx/2.+2.5,
                 dy+6, y-dy/2.-3.5, y+dy/2.+2.5);
    for(Int_t ipix=0; ipix < cluster->GetMultiplicity(); ++ipix)
        h->Fill(pixels[ipix].GetCol(), pixels[ipix].GetRow());
    return h;
}


Bool_t interesting_events_hic(
    const TString filepath_tree,    // path to event tree file
    const TString filepath_ie,      // output path
    const Bool_t  find_ecc = kTRUE, // find empty core clusters
    const Short_t find_mult = 300,  // clusters with multiplicity > <int_mult> are considered interesting (0 = disabled)
    const Short_t find_prox = 0,    // minimum distance to consider cluster fully separate (0 = disabled)
    const Short_t max_mult = 500    // max drawn cluster multiplicity
    ) {

    cout << "interesting_events() : Starting..." << endl;
    cout << " This can take a while depending on interesting criteria and number of events." << endl;

    const Short_t scols    = 1024;
    const Short_t srows    = 512;
    const Short_t n_chips   = 9;   // number of chips

    TChain *chain = new TChain("event_tree", "event_tree");
    if(!chain->Add(filepath_tree.Data())) {
        cerr << "interesting_events() : ERROR: Cannot find event tree in file! " << filepath_tree.Data() << endl;
        return kFALSE;
    }
    BinaryEvent* event = new BinaryEvent();
    chain->SetBranchAddress("event", &event);
    Long_t nentries = chain->GetEntries();
    Long_t cnt=0;

    TFile* file_ie = new TFile(filepath_ie.Data(), "RECREATE");
    if(!file_ie->IsOpen()) {
        cerr << "interesting_events() : ERROR: Cannot open interesting events file! " << filepath_ie.Data() << endl;
        return kFALSE;
    }
    // interesting reasons
    const Short_t n_reasons = 3;
    const TString reasons[n_reasons] = {"Empty core",
                                        Form("Proximity_%i", find_prox),
                                        Form("Multiplicity_%i", find_mult) };
    for(Short_t r=0; r<n_reasons; ++r) {
        file_ie->mkdir(reasons[r]);
        for(Short_t ichip=0; ichip<n_chips; ++ichip) file_ie->mkdir(Form("%s/Chip %i", reasons[r].Data(), ichip));
    }

    for(Long_t ientry=0; ientry < nentries; ++ientry) {
        chain->GetEntry(ientry);
        if( (ientry+1)%10000 == 0 )
            cout << "interesting_events() : Processed events: " << ientry+1 << " / " << nentries << endl;
        for(Short_t ichip=0; ichip < n_chips; ++ichip) {
            Int_t nclusters = event->GetPlane(ichip)->GetNClustersSaved();
            
            // nearby clusters - proximity
            if(nclusters > 1 && find_prox) {
                for(Short_t i=0; i<nclusters; ++i) {
                    for(Short_t j=i+1; j<nclusters; ++j) {
                        Short_t cmx[2] = {event->GetPlane(ichip)->GetCluster(i)->GetX(),
                                          event->GetPlane(ichip)->GetCluster(j)->GetX()};
                        Short_t cmy[2] = {event->GetPlane(ichip)->GetCluster(i)->GetY(),
                                          event->GetPlane(ichip)->GetCluster(j)->GetY()};
                        if( TMath::Abs(cmx[0]-cmx[1]) < find_prox && TMath::Abs(cmy[0]-cmy[1]) < find_prox ) {
                            ++cnt;
                            file_ie->cd(Form("%s/Chip %i", reasons[1].Data(), ichip));
                            TH2F* hClu1 = draw_cluster(event->GetPlane(ichip)->GetCluster(i), scols, srows);
                            hClu1->SetName(Form("ev_%li_sec_%i_clu_%i_prox_%i", ientry, ichip, i, j));
                            file_ie->Write();
                            delete hClu1;
                            TH2F* hClu2 = draw_cluster(event->GetPlane(ichip)->GetCluster(j), scols, srows);
                            hClu2->SetName(Form("ev_%li_sec_%i_clu_%i_prox_%i", ientry, ichip, j, i));
                            file_ie->Write();
                            delete hClu2;
                        }
                    }
                }
            } // end PROX
                            
            for(Int_t iclu=0; iclu < nclusters; ++iclu) {
                Int_t mult = event->GetPlane(ichip)->GetCluster(iclu)->GetMultiplicity();

                if(find_ecc) {   // empty core clusters
                    BinaryPixel* pixels = event->GetPlane(ichip)->GetCluster(iclu)->GetPixelArray(); 
                    Short_t cmx[2] = {TMath::FloorNint(event->GetPlane(ichip)->GetCluster(iclu)->GetX()),
                                      TMath::CeilNint(event->GetPlane(ichip)->GetCluster(iclu)->GetX())};
                    Short_t cmy[2] = {TMath::FloorNint(event->GetPlane(ichip)->GetCluster(iclu)->GetY()),
                                      TMath::CeilNint(event->GetPlane(ichip)->GetCluster(iclu)->GetY())};
                    Bool_t flagCore = kFALSE;
                    for(Int_t ipix=0; ipix<mult && !flagCore; ++ipix)
                        for(Short_t i=0; i<2 && !flagCore; ++i)
                            for(Short_t j=0; j<2 && !flagCore; ++j)
                            flagCore = (pixels[ipix].GetCol() == cmx[i] &&
                                        pixels[ipix].GetRow() == cmy[j]);
                    if(!flagCore) {// draw
                        ++cnt;
                        file_ie->cd(Form("%s/Chip %i", reasons[0].Data(), ichip));
                        TH2F* hClu = draw_cluster(event->GetPlane(ichip)->GetCluster(iclu), scols, srows);
                        hClu->SetName(Form("ev_%li_sec_%i_clu_%i_ec", ientry, ichip, iclu));
                        file_ie->Write();
                        delete hClu;
                    }
                } // end ECC

                if(find_mult && mult >= find_mult) { // multiplicity higher than find_mult
                    ++cnt;
                    file_ie->cd(Form("%s/Chip %i", reasons[2].Data(), ichip));
                    TH2F* hClu = draw_cluster(event->GetPlane(ichip)->GetCluster(iclu), scols, srows);
                    hClu->SetName(Form("ev_%li_sec_%i_clu_%i_mult", ientry, ichip, iclu));
                    file_ie->Write();
                    delete hClu;
                } // end MULT
                    
            }
        } // END FOR chips
    } // END FOR entries
    
    cout << "Found " << cnt << " interesting events." << endl;
    
    file_ie->Close();
    
    cout << "interesting_events() : Done!" << endl;
    return kTRUE;

}
