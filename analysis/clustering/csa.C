...
//_______________________________________________________________________________________________
Bool_t csa(const TString filepath_raw,       // path to SourceRaw_*_*.dat file
           ...
           const Bool_t  flagRmSingleHotPixClusters =
               kTRUE // do not save clusters with just one pixels which is hot
           )
{
  ...
  for (Short_t i              = 0; i < n_secs; ++i)
    plane[i]                  = new BinaryPlane();
  BinaryCluster *     cluster = new BinaryCluster();
  vector<BinaryPixel> pix_vec;
  pix_vec.reserve(MAX_CS);
  BinaryPixel *pix_arr = new BinaryPixel[MAX_CS];
  BinaryPixel  pix_tmp;

  // statistics histograms
  TH1F *hNPixAll = new TH1F(
      "hNPixAll", "Number of hit pixels per event;Number of hit pixels per event;Frequency", 1000,
      0, 10000);
  ...
  if (flagHot) {
    ...
    if (!file_mask.is_open()) {
      cout << "csa() : ERROR: Unable to open mask file: " << filepath_mask.Data() << endl;
      return kFALSE;
    }
  ...
  while (palpidefsRaw->ReadEvent()) {
#ifdef DEBUG
    cout << "csa() : Processed events: " << evts + 1
         << " / Trigger: " << palpidefsRaw->GetEventCounter() << endl;
#else
    if ((evts + 1) % 1000 == 0) cout << "csa() : Processed events: " << evts + 1 << endl;
#endif
    event->Reset();
    ...
    while (palpidefsRaw->GetNextHit(&col, &row)) {
#ifdef DEBUG
      cout << "csa() : DEBUG: Reading hit " << d_get_hit_cnt << " of expected " << nhits
           << ", col: " << col << " row: " << row << " NClu = " << d_n_clu << " nhits = " << nhits
           << " j = " << j << endl;
      d_get_hit_cnt++;
#endif
      ...
      if (flagHot)
        if (hot_map[col][row]) pix_tmp.SetFlag(0, kTRUE);
    ...
    if (flagHot && flagRmSingleHotPixClusters && cluster->GetNPixels() == 1 &&
        cluster->HasHotPixels())
      cluster->Reset();
    ...
    //            cout << i << " col: " << pix_vec[i].GetCol() << " row: " << pix_vec[i].GetRow() <<
    //            endl;
    //#endif
