...
int AddressToColumn(int ARegion, int ADoubleCol, int AAddress)
{
  ...
  // int LeftRight = ((AAddress % 4) < 2 ? 1:0);       // pALPIDE-1/2: left or right column within
  // the double column
  int LeftRight = (((AAddress % 4) == 1) || ((Address % 4) == 2))
                      ? 1
                      : 0; // pALPIDE-3 / ALPIDE: left or right column within the double column
  Column += LeftRight;
...
int AddressToRow(int ARegion, int ADoubleCol, int AAddress)
{
  int Row =
      AAddress /
      2; // This is OK for the top-right and the bottom-left pixel within a group of 4 (all chips)
  // if ((AAddress % 4) == 3) Row -= 1;   // pALPIDE-1/2: adjust the top-left pixel (not needed for
  // pALPIDE-3/ALPIDE)
  // if ((AAddress % 4) == 0) Row += 1;   // pALPIDE-1/2: adjust the bottom-right pixel (not needed
  // for pALPIDE-3/ALPIDE)
  return Row;
}
