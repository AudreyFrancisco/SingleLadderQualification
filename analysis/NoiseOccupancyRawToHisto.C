...
Bool_t NoiseOccupancyRawToHisto(TString file_path)
{
  ...
  TH2F *h2_hitmap;
  h2_hitmap = new TH2F(Form("h2_hitmap_VBB%2.1f_ITHR%i_VCASN%i_VCASN2%i_VCLIP%i_IRESET%i_VRESETP%i_"
                            "VRESETD%i_IDB%i_IBIAS%i_VCASP%i",
                            conf.VBB, conf.ITHR, conf.VCASN, conf.VCASN2, conf.VCLIP, conf.IRESET,
                            conf.VRESETP, conf.VRESETD, conf.IDB, conf.IBIAS, conf.VCASP),
                       Form("Noise Hit Map, "
                            "VBB%2.1f_ITHR%i_VCASN%i_VCASN2%i_VCLIP%i_IRESET%i_VRESETP%i_VRESETD%i_"
                            "IDB%i_IBIAS%i_VCASP%i; Column; Row",
                            conf.VBB, conf.ITHR, conf.VCASN, conf.VCASN2, conf.VCLIP, conf.IRESET,
                            conf.VRESETP, conf.VRESETD, conf.IDB, conf.IBIAS, conf.VCASP),
  ...
  for (Int_t i_sec = 0; i_sec < NSEC; i_sec++) {
    h_noise_occ[i_sec] =
        new TH1F(Form("h_noiseocc_VBB%2.1f_ITHR%i_VCASN%i_VCASN2%i_VCLIP%i_IRESET%i_VRESETP%i_"
                      "VRESETD%i_IDB%i_IBIAS%i_VCASP%i_sec%i",
                      conf.VBB, conf.ITHR, conf.VCASN, conf.VCASN2, conf.VCLIP, conf.IRESET,
                      conf.VRESETP, conf.VRESETD, conf.IDB, conf.IBIAS, conf.VCASP, i_sec),
                 Form("Noise Occupancy, Sector %i, "
                      "VBB%2.1f_ITHR%i_VCASN%i_VCASN2%i_VCLIP%i_IRESET%i_VRESETP%i_VRESETD%i_IDB%i_"
                      "IBIAS%i_VCASP%i; # excluded pixels; noise occupancy [/event/pixel]",
                      i_sec, conf.VBB, conf.ITHR, conf.VCASN, conf.VCASN2, conf.VCLIP, conf.IRESET,
                      conf.VRESETP, conf.VRESETD, conf.IDB, conf.IBIAS, conf.VCASP),
                 1000, 0, 1000);
    ...
    for (Int_t i_pix = 0; i_pix < 1000; i_pix++) {
      ...
      // cout << i_pix << "\t" << x_max << "\t" << y_max << "\t" << h2_hitmap->GetBinContent(x_max,
      // y_max) << endl;
