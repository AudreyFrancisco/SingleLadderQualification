...
void PrepareHistos()
{
  for (int isec = 0; isec < NSEC; ++isec) {
    delete hThresh[isec];
    hThresh[isec] =
        new TH1F(Form("hThresh%d", isec), Form("Threshold, sector %d", isec), 125, 0., 500.);
    delete hNoise[isec];
...
int FitThresholdTune(const char *fName, bool WriteToFile, int ITH, int VCASN, bool saveCanvas)
{
  PrepareHistos();
  ProcessFile(fName);

  std::cout << "Found " << NPixels << " pixels, i.e." << 524288 - NPixels << " pixels have no hits."
            << std::endl;
  ...
  if (!WriteToFile) {
    ...
    if (saveCanvas) c->SaveAs("threshold_scan.png");
  }
  for (int isec = 0; isec < NSEC; ++isec)
  ...
  if (WriteToFile) {
    ...
    fprintf(fp, "%d %d %d %.1f %.1f %.1f %.1f\n", ITH, VCASN, GoodPixels, hThresh[0]->GetMean(),
            hThresh[0]->GetRMS(), hNoise[0]->GetMean(), hNoise[0]->GetRMS());
    fclose(fp);
  }
