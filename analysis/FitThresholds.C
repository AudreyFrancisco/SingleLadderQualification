...
int FitThresholds(const char *fName, bool WriteToFile, int ITH, int VCASN, bool saveCanvas)
{
  ...
  std::cout << "Threshold: " << hThresh->GetMean() << " +- " << hThresh->GetRMS() << " ( "
            << hThresh->GetEntries() << " entries)" << std::endl;
  ...
  if (true) { // if WriteToFile
    ...
    fprintf(fp, "%d %d %d %.1f %.1f %.1f %.1f\n", ITH, VCASN, GoodPixels, hThresh->GetMean(),
            hThresh->GetRMS(), hNoise->GetMean(), hNoise->GetRMS());
    fclose(fp);
  }
