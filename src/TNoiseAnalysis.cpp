...
void TNoiseAnalysis::WriteNoisyPixels(THic *hic) {
  ...
  for (unsigned int i = 0; i < result->m_noisyPixels.size(); i++) {
    if (!common::HitBelongsToHic(hic, result->m_noisyPixels.at(i)))
      continue;
...
void TNoiseAnalysis::Finalize() {
  for (unsigned int ihic = 0; ihic < m_hics.size(); ihic++) {
    ...
    for (unsigned int ichip = 0; ichip < m_chipList.size(); ichip++) {
      ...
      if (chipResult->m_occ > hicResult->m_maxChipOcc)
        hicResult->m_maxChipOcc = chipResult->m_occ;
    }
