...
TAlpide *THic::GetChipById(int chipId) {
  for (unsigned int i = 0; i < m_chips.size(); i++) {
    if ((m_chips.at(i)->GetConfig()->GetChipId() & 0xf) == (chipId & 0xf))
      return m_chips.at(i);
  }
  return 0;
}
...
bool THic::IsPoweredAnalog() {
  if (m_powerBoard && m_powerBoard->IsAnalogChOn(m_pbMod))
    return true;
  return false;
}

bool THic::IsPoweredDigital() {
  if (m_powerBoard && m_powerBoard->IsDigitalChOn(m_pbMod))
    return true;
  return false;
}
...
void THic::PowerOff() {
  ...
  if (m_powerBoard)
    m_powerBoard->SwitchModule(m_pbMod, false);
...
bool THicIB::ContainsChip(common::TChipIndex idx) {
  ...
  if (((int)idx.boardIndex == m_boardidx) && (idx.chipId >= 0) && (idx.chipId < 10))
    return true;
  return false;
}

bool THicIB::ContainsReceiver(int boardIndex, int rcv) {
  if (boardIndex != m_boardidx)
    return false;
  for (int i = 0; i < 9; i++) {
    if (rcv == m_rcv[i])
      return true;
...
bool THicOB::ContainsChip(common::TChipIndex idx) {
  if (idx.chipId < 7) {
    if (((int)idx.boardIndex == m_boardidx0) && ((int)idx.dataReceiver == m_rcv0))
      return true;
  } else {
    if (((int)idx.boardIndex == m_boardidx8) && ((int)idx.dataReceiver == m_rcv8))
      return true;
  }
...
bool THicOB::ContainsReceiver(int boardIndex, int rcv) {
  if ((boardIndex == m_boardidx0) && (rcv == m_rcv0))
    return true;
  if ((boardIndex == m_boardidx8) && (rcv == m_rcv8))
    return true;
...
int THicOB::GetReceiver(int boardIndex, int chipId) {
  if (boardIndex == m_boardidx0 && (chipId & 0xf) < 8)
    return m_rcv0;
  if (boardIndex == m_boardidx8 && (chipId & 0xf) >= 8)
    return m_rcv8;
