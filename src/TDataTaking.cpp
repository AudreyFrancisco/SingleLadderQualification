...
void TDataTaking::ConfigureFromu(TAlpide *chip)
{
  ...
  chip->WriteRegister(Alpide::REG_FROMU_CONFIG1,
                      data); // digital pulsing with strobe generated by chip (and busy monitoring)
  chip->WriteRegister(
...
void TDataTaking::Execute()
{
  ...
  for (unsigned int iboard = 0; iboard < m_boards.size(); iboard++) {
    TReadoutBoardRUv1 *boardy = dynamic_cast<TReadoutBoardRUv1 *>(m_boards.at(iboard));
    nTriggers                 = (m_value[0] == m_stop[0] - 1) ? m_nLast : kTrigPerTrain;
    int configTrigPerTrain    = m_config->GetParamValue("MAXNTRIGTRAIN");
    if (boardy) nTriggers = (m_value[0] == m_stop[0] - 1) ? m_nLast : configTrigPerTrain;
