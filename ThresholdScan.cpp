...
int main() {
  ...
  for (std::vector<TAlpide*>::iterator ichip = fChips.begin(); ichip != fChips.end(); ichip++) {
    ...
    AlpideConfig::WritePixRegAll           (*ichip, Alpide::PIXREG_MASK, false);
    AlpideConfig::WritePixRegAll           (*ichip, Alpide::PIXREG_SELECT, false);
  ...
  for (int irow = 0; irow < 512; irow ++) {
    // row selection
    for (std::vector<TAlpide*>::iterator ichip = fChips.begin(); ichip != fChips.end(); ichip++) {
      if (irow > 0) AlpideConfig::WritePixRegRow (*ichip, Alpide::PIXREG_SELECT, false, irow -1);
      AlpideConfig::WritePixRegRow (*ichip, Alpide::PIXREG_SELECT, true, irow);
    }
