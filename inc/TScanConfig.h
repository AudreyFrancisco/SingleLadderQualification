...
namespace ScanConfig {
  ...
  const int SCAN_STEP   = 1; // Grab every Xth row (for tuneITHR/VCASN scan only).
  ...
  // MFT ThresholdScan : only ~3% pixels tested 1024*512 - 32*512 = 507904 apparently dead.
  // TODO: Define cut values for MFT ThresholdScan
  ...
  const int EYE_DRIVER = 4;
  const int EYE_PREEMP = 4;
