...
class TDigitalWFAnalysis : public TScanAnalysis {
 ...
 protected:
  TScanResultChip *GetChipResult () {TDigitalWFResultChip *Result = new TDigitalWFResultChip(); return Result;};
  TScanResultHic  *GetHicResult  () {TDigitalWFResultHic  *Result = new TDigitalWFResultHic (); return Result;};
  void             CreateResult  () {};
  void             AnalyseHisto  (TScanHisto *histo);
 public:
  TDigitalWFAnalysis(std::deque<TScanHisto> *histoQue, 
                   TScan                  *aScan, 
                   TScanConfig            *aScanConfig, 
                   std::vector <THic*>     hics,
                   std::mutex             *aMutex, 
                   TDigitalWFResult       *aResult = 0);
