...
class AlpideDecoder {
 private:
   ...
   static void      DecodeDataWord     (unsigned char *data, int chip, int region, std::vector <TPixHit> *hits, bool datalong, int channel, int &prioErrors, std::vector <TPixHit> *stuck);
 protected:
 public:
   static TDataType GetDataType        (unsigned char dataWord);
   static int       GetWordLength      (TDataType dataType);
   static bool      DecodeEvent        (unsigned char *data, int nBytes, std::vector <TPixHit> *hits, int channel, int &prioErrors, std::vector <TPixHit> *stuck=0);
